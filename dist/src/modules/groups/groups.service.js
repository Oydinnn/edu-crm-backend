...
let GroupsService = class GroupsService {
    ...
    async getLessonsByGroupId(groupId, userId) {
        ...
        let lessonsFormatted = lessons.map(lesson => {
            return {
                ...
                homework_id: lesson.homework[0]?.id ? lesson.homework[0].id : null,
                videoCount: lesson._count.lessonVideos || 0,
            };
        });
    ...
    async getSchedules(groupId, userId, role) {
        const group = await this.prisma.group.findUnique({
            where: { id: groupId },
            include: { courses: true, groupTeachers: true },
        });
        if (!group) {
            throw new common_1.NotFoundException("Group not found");
        }
        const teacherId = group.groupTeachers?.find(gt => gt.teacher_id === userId)?.teacher_id;
        if (role === client_1.Role.TEACHER && !teacherId) {
            throw new common_1.ForbiddenException("You don't have access to this group");
        }
        const weekDayMap = {
            SUNDAY: 0,
            MONDAY: 1,
